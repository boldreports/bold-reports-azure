                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.Data) {
                            ...
                            $("#report_name_validation_error").html(window.Server.App.LocalizationContent.ReportNameAlreadyExist);
                            isAlreadyExist = true;
                            return;
                        }
                    }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.result.Status) {
                            closeNewRDLAddPopup();
                            parent.messageBox("su-report", window.Server.App.LocalizationContent.UpdateReport, window.Server.App.LocalizationContent.ReportUpdateMessage, "success", function () {
                            ...
                            closeNewRDLAddPopup();
                            parent.messageBox("su-report", window.Server.App.LocalizationContent.UpdateReport, window.Server.App.LocalizationContent.ErrorUpdatingReport, "success");
                        }
                    },
                    error: function (e) {
                        window.parent.$("#report_popup_wrapper").ejWaitingPopup("hide");
                ...
                if (dataSourceList.length !== 0 || datasetList.length !== 0) {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.result.Status) {
                                closeNewRDLAddPopup();
                                parent.messageBox("su-report", window.Server.App.LocalizationContent.AddReport, window.Server.App.LocalizationContent.ReportAddedSucessfully, "success", function () {
                                    if (typeof (parent.RefreshCategorySection) === "function") {
                                        parent.RefreshCategorySection(categoryId);
                                    }
                                ...
                                closeNewRDLAddPopup();
                                parent.messageBox("su-report", window.Server.App.LocalizationContent.AddReport, window.Server.App.LocalizationContent.ReportAddedFailed, "success");
                            }
                        },
                        error: function (e) {
                            window.parent.$("#report_popup_wrapper").ejWaitingPopup("hide");
...
function onSelectionOfDataSource(element, object) {
    ...
    $(element).find(".select-datasource-btns").attr("value", window.Server.App.LocalizationContent.UpdateButton);
...
function onSelectionOfDataset(element, object) {
    ...
    $(element).find(".select-dataset-btns").attr("value", window.Server.App.LocalizationContent.UpdateButton);
...
function reportUploadComplete(reportUploadStatus) {
    if (reportUploadStatus.Status) {
        $("#uploadValidation").html("<span class='ReportUploadSuccessMessage'>" + window.Server.App.LocalizationContent.ReportUploadSuccess + "</span>");
        ...
        if (reportUploadStatus.IsSharedDataSource) {
            ...
            for (var dataSource = 0; dataSource < reportUploadStatus.SharedDataSourceList.length; dataSource++) {
                html += '<tr data-key="' + reportUploadStatus.SharedDataSourceList[dataSource] + '"><td><input type="button" class="btn primary-button select-datasource-btns small-left-margin" value="' + window.Server.App.LocalizationContent.SelectButton + '" data-name="' + reportUploadStatus.SharedDataSourceList[dataSource] + '"></td><td><span class="rdl-datasource-name">' + reportUploadStatus.SharedDataSourceList[dataSource] + '</span></td><td> :</td><td><span class="selected-datasource" style = "max-width: 527px; overflow:hidden; text-overflow:ellipsis; display:inline-block; white-space:nowrap; vertical-align:middle; padding-left:5px; max-width: 527px;" ></span></td></tr>';
            }
            $("#datasource_list_table").html(html);
            $("#datasource_list").show();
        }
        ...
        if (reportUploadStatus.IsSharedDataSet) {
            ...
            for (var dataset = 0; dataset < reportUploadStatus.SharedDataSetList.length; dataset++) {
                html += '<tr data-key="' + reportUploadStatus.SharedDataSetList[dataset] + '"><td><input type="button" class="btn btn-primary select-dataset-btns small-left-margin" value="' + window.Server.App.LocalizationContent.SelectButton + '" data-name="' + reportUploadStatus.SharedDataSetList[dataset] + '"></td><td><span class="rdl-dataset-name">' + reportUploadStatus.SharedDataSetList[dataset] + '</span></td><td> :</td><td><span class="selected-dataset" style = "max-width: 527px; overflow:hidden; text-overflow:ellipsis; display:inline-block; white-space:nowrap; vertical-align:middle; padding-left:5px; max-width: 527px;" ></span></td></tr>';
            }
            $("#dataset_list_table").html(html);
            $("#dataset_list").show();
        }
        ...
        $("#uploadValidation").html("<span class='ErrorMessage'>" + window.Server.App.LocalizationContent.ErrorUploadingReport + "</span>");
    }
}
