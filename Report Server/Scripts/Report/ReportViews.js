...
function CloseReportView() {
    ...
    $("#sync_report_viewer").css("width", $(window).width());
    $("#expand-collapse").html('<span class="su su-sidebar-collapse"></span>');
    $("#expand-collapse").attr("data-original-title", window.Server.App.LocalizationContent.ReportViews);
    $("#report-view-toogle").toggleClass("report-view-toogle");
}
...
function AddView() {
    ...
    var itemRequest = { name: _data, ItemViewName: SaveFilterName, QueryString: queryString, itemId: item_Id, UserId: userId, UserName: userName, itemType: reportItemDetail.ItemType};
    ajaxPostCall("POST", addViewUrl, itemRequest, null, saveViewResult, errorSaveView, null);
}
...
function saveViewResult(data) {
    ...
    if (!data.Status) {
        messageBox("su-filter", window.Server.App.LocalizationContent.AddView, window.Server.App.LocalizationContent.ViewAlreadyExists, "success", function () {
            onCloseMessageBox();
        });
        $('#filter-name-input').addClass("has-error");
    }
...
function GetSavedFilter() {
    var reportViewDiv = document.getElementById("report-views");
    reportviewerObj = $("#sync_report_viewer").data("boldReportViewer");
    var itemRequest = { itemId: item_Id, UserId: userId, UserName: userName, itemType: reportItemDetail.ItemType};
...
function getViewsResult(data) {
    $('#saved-filter').html("");
    var savedFilter =
        '<div id="save-view-label"><label class="save-filter-head">' + window.Server.App.LocalizationContent.SavedViews + '</label></div><div id="saved-list" style="display: block"><div class="saved-list-content-div" style="float: left">'
    ...
    if (Result.length > 0) {
        ...
        for (var i = 0; i < Result.length; i++) {
            ...
            if (Result[i].CanDelete && Result[i].CanShare && !embedConfig.IsEmbedCode ) {
                savedFilterOptions +=
        ...
        $("#report-view-toogle").length > 0 ? $("#report-view-toogle").find("#saved-list").length == 0 ? $("#report-view-toogle").find("#no-filters").css("display", "block") : $("#report-view-toogle").find("#no-filters").css("display", "none") : parent.$("#report-view-toogle").find("#saved-list").length == 0 ? parent.$("#report-view-toogle").find("#no-filters").css("display", "block") : parent.$("#report-view-toogle").find("#no-filters").css("display", "none");
        if (/Safari/.test(navigator.userAgent) && /Apple Computer/.test(navigator.vendor)) {
            $(".View-link-copy").removeClass("su su-link");
            $(".View-link-copy").attr("data-original-title", "");
        }
        $("#report-view-toogle").length > 0 ? $("#report-view-toogle").show() : parent.$("#report-view-toogle").show();
        $('[data-toggle="tooltip"]').tooltip();
    } else {
...
$(document).on("click", "#update-view", function () {
    ...
    var proxy = $('#sync_report_viewer').data('boldReportViewer');
    var queryString = saveQueryString != "" ? saveQueryString : QueryStringFormation(false);
    var itemRequest = { itemId: item_Id, QueryString: queryString, itemViewId: $(this).attr("viewid"), UserId: userId, UserName: userName, itemType: reportItemDetail.ItemType };
...
$(document).on("click", "#delete_item", function () {
    ...
    var itemRequest = { ItemViewName: $("#filter_name").val(), itemId: item_Id, itemViewId: $(this).attr("viewid"), UserId: userId, UserName: userName, itemType: reportItemDetail.ItemType };
...
function messageBox(messageIcon, messageHeader, messageText, type, successCallback, errorCallback, width, height, maxHeight) {
    ...
    if (type == "error") {
        ...
        if (successCallback != undefined) {
            successButton = $("<input type='button' class='critical-action-button pull-right' value='" + window.Server.App.LocalizationContent.Yes + "'></input>");
            successButton.bind("click", $.proxy(getFnObj(successCallback), window));
        }
        if (errorCallback != undefined) {
            errorButton = $("<input type='button' class='secondary-button pull-right' value='" + window.Server.App.LocalizationContent.No + "'></input>");
        ...
        var closeIcon;
        if (successCallback != undefined) {
            successButton = $("<input type='button' class='secondary-button' value='" + window.Server.App.LocalizationContent.Ok + "'></input>");
            successButton.bind("click", $.proxy(getFnObj(successCallback), window));
            closeIcon = $('<span class="su su-close"></span>');
