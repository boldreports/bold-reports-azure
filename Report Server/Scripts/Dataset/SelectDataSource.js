$(function () {
    ...
    }, window.Server.App.LocalizationContent.CredentialRequired);
    ...
    switch (activeTab) {
        case "avl_datasource_tab":
            if ($("#available_datasource_table").find(".available-datasource-select").length == 0) {
                ...
                $("#datasource_validation").html(window.Server.App.LocalizationContent.SelectDatasource).css("display", "block");
...
function onAvailableDataSourceTabSelect() {
    ...
    $("#rdl_new_tab_arrow").css("left", "50px");
    $("#publish_datasource").attr("value", window.Server.App.LocalizationContent.SelectButton);
}
...
function onNewDataSourceTabSelect() {
    ...
    $("#publish_datasource").attr("value", window.Server.App.LocalizationContent.AddButtonDatasource);
...
function validateNewDataSourceForm() {
    ...
    if (dataSourceName == "") {
        $("#datasource_name_validation_error").html(window.Server.App.LocalizationContent.NameNotEmpty);
        isValid = false;
    }
    else if (!parent.IsValidName("name",dataSourceName)) {
        $("#datasource_name_validation_error").html(window.Server.App.LocalizationContent.NamingRules);
        isValid = false;
    }
    ...
    if (connectUsing != "None" && connectionString == "" && $("#datasource-type").val() != "XML") {
        $("#datasource_connstring_validation_error").html(window.Server.App.LocalizationContent.EmptyConnectionString);
        isValid = false;
    }
    ...
    if (connectUsing == "Store") {
        if (window.isEditPopup) {
            ...
            if (storedUserName == "" || storedPassword == "") {
                $("#datasource_credential_validation_error").html(window.Server.App.LocalizationContent.CredentialRequired);
                isValid = false;
            }
        }
    }
...
function closeNewDataSourcePopup(e) {
    ...
    $("#prompt_text").val(window.Server.App.LocalizationContent.DatasourceAuthentication);
    window.parent.$('#select_datasource_popup_wrapper').ejWaitingPopup("hide");
