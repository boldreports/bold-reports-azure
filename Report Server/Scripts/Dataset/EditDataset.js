$(function () {
    ...
    $("#editItemForm").validate({
        ...
        messages: {
            "Name": {
                isRequired: window.Server.App.LocalizationContent.DatasetName
            }
        },
...
function datasetUploadComplete(datasetUploadStatus) {
    if (datasetUploadStatus.Status) {
        $("#uploadValidation").html("<span class='ReportUploadSuccessMessage'>" + window.Server.App.LocalizationContent.UploadDatasetSuccess +"</span>");
        ...
        if (datasetUploadStatus.IsSharedDataSource) {
            ...
            for (var dataSource = 0; dataSource < datasetUploadStatus.SharedDataSourceList.length; dataSource++) {
                html += '<tr data-key="' + datasetUploadStatus.SharedDataSourceList[dataSource] + '"><td><input type="button" class="btn primary-button select-datasource-btns small-left-margin" value="' + window.Server.App.LocalizationContent.SelectButtonsDataset +'" data-name="' + datasetUploadStatus.SharedDataSourceList[dataSource] + '"></td><td><span class="rdl-datasource-name">' + datasetUploadStatus.SharedDataSourceList[dataSource] + '</span></td><td> :</td><td><span class="selected-datasource" style = "overflow:hidden; text-overflow:ellipsis; display:inline-block; white-space:nowrap; vertical-align:middle; padding-left:5px; max-width: 435px; width: auto" ></span></td></tr>';
            }
        ...
        $("#uploadValidation").html("<span class='ErrorMessage'>" + window.Server.App.LocalizationContent.ErrorUploadingDataset +"</span>");
    }
}
...
function EditDataset() {
    ...
    if ($("#publishedFileName").attr("value") == "" || $("#publishedFileName").attr("value") == "none") {
        ...
        $("#uploadValidation").html(window.Server.App.LocalizationContent.UploadDatasetMessage);
        isValidForm = false;
    } else {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.Data) {
                        ...
                        $("#validate-name").html(window.Server.App.LocalizationContent.DatasetIsAlreadyExist);
                        isAlreadyExist = true;
                        return;
                    }
                }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.result.Status) {
                        closeNewDatasetAddPopup();
                        parent.messageBox("su-dataset", window.Server.App.LocalizationContent.UpdateDataset, window.Server.App.LocalizationContent.UpdateDatasetSuccess, "success", function () {
                        ...
                        closeNewDatasetAddPopup();
                        parent.messageBox("su-report", window.Server.App.LocalizationContent.UpdateDataset, window.Server.App.LocalizationContent.FailedUpdateDatasetMessage, "success");
                    }
                },
                error: function (e) {
                    window.parent.$("#addDataSetDom_wrapper").ejWaitingPopup("hide");
