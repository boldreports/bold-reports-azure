...
function addItemValidate() {
    if ($("#addCategoryForm").valid()) {
        ...
        if ((isSelfHosted && tenantName == currentSite) || (!isSelfHosted)) {
        ...
        else if (currentSite != tenantName) {
            parent.window.CategoryName = categoryName;
            $.ajax({
                type: "POST",
                url: addCategoryTenantUrl,
                data: { CategoryName: categoryName, Description: categoryDescription, TenantName: tenantName },
                success: function (data) {
                    if (!isCategoryPopupInReportDesigner) {
                        if (data.IsSuccess) {                            
                            var partialItemWaitingPopupTemplateId = parent.createLoader("item-list-panel");
