        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.Success) {
                    ...
                    else if (itemtype != undefined && itemtype.toLowerCase() == "dataset") {
                        if (data.DatasetCount != undefined && data.DatasetCount > 0) {
                            onSuccessDeleteItem();
                        }
                        else {
                            parent.$("#body").ejWaitingPopup("show");
...
function fnActionComplete(args) {
    ...
    if (gridObj._gridRecordsCount == 0) {
        if (gridName = $('#item-grid-container').attr("data-grid-name") == 'datasources') {
            ...
            var message = "";
            message = window.Server.App.LocalizationContent.NoDatasets;
        }
...
$(document).on("click", "#open-dashboard, #open-widget", function (e) {
    if ($(this).attr("id") == "open-dashboard") {
        if ($(this).attr("data-href").indexOf("?") == -1) {
            window.open($(this).attr("data-href") + "?showmyreports=1", "_blank");
        }
        else {
            window.open($(this).attr("data-href") + "&showmyreports=1", "_blank");
        }
...
$(document).on("click", ".open-item", function (e) {
    var url = $(this).attr("href") + "?showmyreports=1";
