...
shareItem.controller('shareItemController', ["$scope", function ($scope) {
    ...
    $scope.GetManagePermisisionContent = function (index) {
        var content = window.Server.App.LocalizationContent.RemoveAccess;
        if ($scope.IsUserItem(index)) {
            content = content + window.Server.App.LocalizationContent.PermissionGlobally + " - <strong>" + sharedUserGroupsPreservedList[index].PermissionEntityDescription + "</strong>.</br></br>";


            if (isAdmin) {
                var userEditLink = editUserPermissionUrl + "?userId=" + sharedUserGroupsPreservedList[index].TargetUserId;
                return content + window.Server.App.LocalizationContent.ManagePermission + " <a target='_blank' href='" + userEditLink + "'>" + window.Server.App.LocalizationContent.Here + "</a>."
            } else {
                return content + window.Server.App.LocalizationContent.UserContactSystemAdministrator;
            }

        } else {
            content = content + window.Server.App.LocalizationContent.AssignedGlobally + " - <strong>" + sharedUserGroupsPreservedList[index].PermissionEntityDescription + "</strong>.</br></br>";

            if (isAdmin) {
                var groupEditLink = editGroupPermissionUrl + "?groupId=" + sharedUserGroupsPreservedList[index].TargetGroupId;
                return content + window.Server.App.LocalizationContent.ManagePermission + " <a target='_blank' href='" + groupEditLink + "'>" + window.Server.App.LocalizationContent.Here + "</a>."
            } else {
                return content + window.Server.App.LocalizationContent.GroupContactSystemAdministrator;
            }
        }
    }
}]);
...
function generateEmbedCode(itemDetail) {
    ...
    var embedFrameCode = "<iframe src='" + embedDashboardUrl + "' id='report-frame' width='100%' height='720px' allowfullscreen frameborder='0'></iframe>";
