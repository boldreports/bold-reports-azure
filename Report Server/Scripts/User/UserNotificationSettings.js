...
$(document).ready(function () {
    ...
    $(document).on("click", "#edit-settings", function (e) {
        updatedNotificationSettings = {
            ...
            EnableReportScheduleNotification: parseInt($("#report-schedule-notification").val()),
            EnableUserScheduleNotification: parseInt($("#user-schedule-notification").val())
        };
    ...
    $(document).on("click", "#update-notification-settings", function () {
        ...
        if (isAdmin)
        {
            notificationSettings.EnableUserScheduleNotification = parseInt($("#user-schedule-notification").val());
        }
        ...
        $.ajax({
            ...
            success: function (result) {
                if(result.Status)
                {
                    ...
                    if (isAdmin)
                    {
                        updatedNotificationSettings.EnableUserScheduleNotification = parseInt($("#user-schedule-notification").val());
                    }
...
function cancelSettings(updatedNotificationSettings) {
    ...
    $("#report-schedule-notification").val(updatedNotificationSettings.EnableReportScheduleNotification);
    $("#user-schedule-notification").val(updatedNotificationSettings.EnableUserScheduleNotification);
    $(".notification-disabled").attr("disabled", true).selectpicker("refresh");
    $(".selectpicker").removeClass("enable");
    $("#edit-settings").show();
    $("#update-notification-settings").hide();
