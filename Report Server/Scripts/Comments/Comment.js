var cursorPos, Paste, createHiddenEditable, isFocusable, initialLine, initialPosition, targetElement, itemId, dataManger, elementNode, getAllCommentAjax, deletedCommentId;
...
$(document).ready(function () {
    ...
    function OnCommentPosted(data) {
        if (data) {
            var addComment = "<li id='comment-"
                ...
                + data.Comments[0].UserDisplayName + "' data-image-size='32' data-toggle='tooltip' title='"
                + data.Comments[0].UserDisplayName + "' data-placement='top'></div> </div></div><div class='post-body'> <header><span class='post-byline col-xs-5 no-padding'><span title='"
    ...
    $(document).on('click', '.delete-comment', function () {
        ...
        parent.messageBox("su-delete", window.Server.App.LocalizationContent.DeleteComment, window.Server.App.LocalizationContent.DeleteComentConfirm, "error", function () {
            ...
            var itemRequest = { itemId: itemId, commentId: commentId, itemType: itemType, dashboardItemId: dashboardItemId, url: decodeURI(currentPageUrl), commentAction: commentAction.Delete, currentDate: date };
        ...
        for (var i = 0; i < item.length; i++) {
            ...
            if (!item[i].IsActive) {
                commentHtml += "<li id='comment-"
                    ...
                    + item[i].UserDisplayName + "' data-image-size='32' data-toggle='tooltip' title='"
                    + item[i].UserDisplayName + "' data-placement='top'></div></div><div class='post-body'> <header><span class='post-byline col-xs-5 no-padding'></span><span class='post-meta col-xs-9 hidden no-padding'><a " + commentPermalinkHelper(item[i].Id) + " class='permalink'><span class='time-ago' title='"
                ...
                commentHtml += "<li id='comment-"
                    ...
                    + item[i].UserDisplayName + "' data-image-size='32' data-toggle='tooltip' title='"
                    + item[i].UserDisplayName + "' data-placement='top'></div></div></div><div class='post-body'> <header><span class='post-byline col-xs-5 no-padding'><span title='"
