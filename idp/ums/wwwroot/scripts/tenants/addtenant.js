...
$(document).ready(function () {
    ...
    function OpenHandler(args) {
        ...
        if (isCommonLogin) {
            if (!IsDefaultSchemaUms || !IsDefaultPrefixUms) {
                if (Lielement[0].innerText === 'Enterprise BI') {
                    Lielement[0].classList.add('e-disabled');
                    Lielement[0].style.pointerEvents = 'none';
                }
            }
            else {
                if (Lielement[0].innerText === 'Enterprise BI') {
                    Lielement[0].classList.remove('e-disabled');
                    Lielement[0].style.pointerEvents = '';
                }
            }
        }
    }
    ...
    var dropTenantTypeDownList = new ejs.dropdowns.DropDownList({
        index: isIndexForCustomPrefix ? 1 : 0,
