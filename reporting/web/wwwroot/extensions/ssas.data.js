var SASSDataSource = (function () {
    ...
    SASSDataSource.prototype.renderSassPanel = function () {
        ...
        this.savePasswrdTag = this.sassPanel.find('#' + this.id + '_psql_save_password_tr');
        this.ejSassSavePasswrd = this.sassPanel.find('#' + this.id + '_psql_save_password').data('ejCheckBox');
    };
    ...
    SASSDataSource.prototype.updateDataSource = function (dataSource) {
        if (!ej.isNullOrUndefined(dataSource)) {
            ...
            this.ejSassSavePasswrd.option('checked', dataSource.ConnectionProperties.EmbedCredentials);
            if (connectionProperties.IntegratedSecurity) {
                ...
                if (!dataSource.ConnectionProperties.IntegratedSecurity &&
                    ...
                    this.ejAuthDrpdwn.setModel({ 'text': this.getLocale('sqlServer') });
                    this.ejSassSavePasswrd.option('checked', dataSource.ConnectionProperties.EmbedCredentials);
                }
                else if (!dataSource.ConnectionProperties.IntegratedSecurity &&
                    ...
                    (dataSource.SecurityType.toString() === '2' || dataSource.SecurityType === 'DataBase')) {
                    this.ejAuthDrpdwn.setModel({ 'text': this.getLocale('sqlServer') });
                    this.ejSassSavePasswrd.option('checked', false);
                }
                else if (!dataSource.ConnectionProperties.IntegratedSecurity && dataSource.ConnectionProperties.Prompt
                    && dataSource.ConnectionProperties.Prompt.length > 0) {
                    ...
                    this.promptCont.val((!ej.isNullOrUndefined(dataSource.ConnectionProperties.Prompt)
                        && dataSource.ConnectionProperties.Prompt.length > 0)
                        ? dataSource.ConnectionProperties.Prompt : this.getLocale('connPrompt') + dataSource.Name);
                    this.ejSassSavePasswrd.option('checked', false);
                }
                else if (dataSource.ConnectionProperties.IntegratedSecurity) {
                    this.ejAuthDrpdwn.setModel({ 'text': this.getLocale('window') });
                }
            }
        }
    };
    ...
    SASSDataSource.prototype.resetInputFields = function () {
        ...
        this.hideValidationMsg();
        this.ejAuthDrpdwn.selectItemByValue('windows');
        this.ejSassSavePasswrd.setModel({ 'checked': false });
    };
    SASSDataSource.prototype.getDatasourceInfo = function (dataSourceName, dataSource) {
        ...
        if (isValidCon) {
            ...
            if (this.ejAuthDrpdwn.getSelectedValue() === 'sqlServer' && this.ejSassSavePasswrd.model.checked) {
    ...
    SASSDataSource.prototype.getLocale = function (text) {
        ...
        if (!ej.isNullOrUndefined(SASSDataSource.Locale[this.locale])) {
        ...
        switch (text.toLowerCase()) {
            ...
            case 'savepassword':
                if (sassLocale && sassLocale.savePassword) {
                    return sassLocale.savePassword;
                }
    ...
    SASSDataSource.prototype.updateCulture = function (culture) {
        this.locale = culture;
        var savePassword = this.sassConfig.find('#' + this.id + '_psql_save_password');
        if (this.connString) {
        ...
        if (this.authType) {
            ...
            this.ejAuthDrpdwn.setModel({ 'dataSource': this.getDropdownValues(), 'selectedIndex': selectedIndex });
        }
        if (this.promptCont) {
        ...
        if (savePassword) {
            this.updateRow(this.sassConfig, savePassword.attr('id'), this.getLocale('savepassword'));
        }
    };
    ...
    SASSDataSource.prototype.renderErrIndictor = function (target, tooltipId, errMsg) {
        var errorIcon = ej.buildTag('span.e-rptdesigner-error-icon e-rptdesigner-errorinfo e-ssas-error-tip', '', {
            'float': 'right',
            'display': 'none',
            'padding-right': '2px'
        }, {
    ...
    SASSDataSource.prototype.showErrIndictor = function (target, isEnable, errMsg) {
        var errorIcon = target.find('.e-ssas-error-tip');
        ...
        if (isEnable) {
            ...
            ejTooltip.setModel({
                target: '.e-rptdesigner-error-icon.e-ssas-error-tip',
            });
        }
    };
    ...
    SASSDataSource.prototype.updateValidationMsg = function (target, id, msg) {
        var toolTipCont = target.find('#' + id + '_error_icon_td .e-ssas-error-tip');
